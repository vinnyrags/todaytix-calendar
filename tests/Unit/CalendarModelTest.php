...
final class CalendarModelTest extends TestCase
{
    ...
    public function testTimeFormatOverridesTheLabel(): void
    {
        // View wants "7:00" (no meridiem) instead of the default "7 PM".
        $run   = [new Showtime(1, $this->dt('2026-12-20 19:00'), Availability::AVAILABLE)];
        $model = new CalendarModel($this->et, $this->dt('2026-12-01'), new BuyLinkBuilder('https://x.test', 1), 0, [], ['available', 'limited'], true, 'g:i');
        $dec20 = $this->findDay($model->build($run, $this->dt('2026-12-01'), $this->dt('2026-12-01'))[0], '2026-12-20');

        self::assertSame('7:00', $dec20->performances[0]['time']);
    }
